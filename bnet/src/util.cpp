...
const char * const BITCOIN_CONF_FILENAME = "bnet.conf";
const char * const BITCOIN_PID_FILENAME = "bnetd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bnet" is a composite category enabling all Bnet-related debug output
            if(ptrCategory->count(string("bnet"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BnetCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BnetCore
    // Mac: ~/Library/Application Support/BnetCore
    // Unix: ~/.bnetcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BnetCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BnetCore";
#else
    // Unix
    return pathRet / ".bnetcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bnet.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bnet.conf
