...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Bnet Core server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "Bnet Core server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Bnet features */
    { "bnet",               "masternode",             &masternode,             true  },
    { "bnet",               "masternodelist",         &masternodelist,         true  },
    { "bnet",               "masternodebroadcast",    &masternodebroadcast,    true  },
    { "bnet",               "gobject",                &gobject,                true  },
    { "bnet",               "getgovernanceinfo",      &getgovernanceinfo,      true  },
    { "bnet",               "getsuperblockbudget",    &getsuperblockbudget,    true  },
    { "bnet",               "voteraw",                &voteraw,                true  },
    { "bnet",               "mnsync",                 &mnsync,                 true  },
    { "bnet",               "spork",                  &spork,                  true  },
    { "bnet",               "getpoolinfo",            &getpoolinfo,            true  },
#ifdef ENABLE_WALLET
    { "bnet",               "privatesend",            &privatesend,            false },
...
{
    return "> bnet-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:9457/\n";
}
